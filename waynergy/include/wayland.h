...
struct wlContext {
	 ...
	 * or -1 to disable */
	int uinput_fd[2];
	/* Optional local datagram feed for a compositor-side pointer indicator. */
	char *pointer_socket;
...
/* Optional non-input pointer-position feed. */
extern bool wlPointerFeedInit(struct wlContext *context);
extern void wlPointerFeed(struct wlContext *context, int x, int y);
extern void wlPointerFeedResync(struct wlContext *context);
