...
static void syn_active_cb(uSynergyCookie cookie, bool active)
{
	...
	if (active) {
		/* CINN may arrive before another mouse event.  Re-publish the last
		 * pointer position so the compositor can reapply its saved mapping. */
		wlPointerFeedResync(&wlContext);
	} else {
...
int main(int argc, char **argv)
{
	...
	wlContext.pointer_socket = configTryString("pointer/socket", NULL);
	wlContext.pointer_socket_fd = -1;
...
opterror:
	...
	if (!wlPointerFeedInit(&wlContext))
		goto error;
